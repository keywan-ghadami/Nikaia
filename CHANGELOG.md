...
- **`examples/`**: specification-level programs that the bootstrap parser cannot compile yet, kept separate from `tests/samples/` (which must parse and is covered by `cargo test`).
  - `1brc.nika` — the One Billion Row Challenge: a scannerless grammar for the line format with a commit point after the separator, station names as tethered slices into the input buffer, integer-tenths aggregation, and a `sync` fold distributed with `par_iter`. Chosen because it stresses precisely what ADR-007 asserts — if it is slow, the grammar protocol's performance argument is wrong.
  ...
  - `README.md` — survey of the benchmark suites worth targeting (1BRC, Computer Language Benchmarks Game, TechEmpower, Are We Fast Yet?), with a recommended order, plus the seven specification gaps (G1–G7) that writing the two programs exposed.
