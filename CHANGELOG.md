...
- **Spec Part III (17.1)**: `fs::map` is a **compile-time error under the Lite profile**, in the same way `std::thread` is. WASM has no memory mapping, and silently degrading it to a full read would turn a constant-memory program into one that allocates its entire input.
...
- **`examples/`**: specification-level programs that the bootstrap parser cannot compile yet, kept separate from `tests/samples/` (which must parse and is covered by `cargo test`).
  ...
  - `README.md` — survey of the benchmark suites worth targeting (1BRC, Computer Language Benchmarks Game, TechEmpower, Are We Fast Yet?), with a recommended order, plus the specification gaps writing the two programs exposed. G1 (`std::fs`) is resolved above. G2 was restated: the original wording claimed a gap around "applying a grammar per line", which misread the language — the real gap is a **folding repetition** at the entry rule, so a parser can thread an accumulator instead of collecting a billion nodes.
