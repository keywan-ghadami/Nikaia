...
**Pre-alpha, as of 0.0.207.** The [roadmap](docs/project_status_and_roadmap.md) shows 74 % —
...
| hand a **view** of text to a **published** (`pub`) function that also gets text of its own for the same `String` parameter | refused, and the message says why and what copies nothing | a published signature is fixed before its callers exist. Everywhere else a declared `String` or `String?` — field, result, parameter, `let`, the elements of a `Vec` or a map, from inside an `f"…"` hole too — becomes a view, or either per value, by what flows into it ([ADR-222](docs/specification/adr/adr-222.md), [223](docs/specification/adr/adr-223.md), [224](docs/specification/adr/adr-224.md)) |
