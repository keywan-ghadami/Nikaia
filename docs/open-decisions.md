...
Nothing. The last question here — *a container of structs holding views that
drops entries in a loop* — was answered by
[ADR-221](specification/adr/adr-221.md): option 3, each element held beside the
buffers it points into, one handle per buffer.
