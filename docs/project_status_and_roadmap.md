...
*   [ ] **Tier-1 staging (compiler-side)**: independent of compile-time I/O and blocked by none of its questions ([ADR-026](specification/adr/adr-026.md) §3). Survey of real candidates, closed ones, and how to measure one: [`docs/staging-candidates.md`](staging-candidates.md). Headline findings: route hashing has no target (no HTTP server); the biggest win - dispatch over literal alternations - lives in the `winnow-grammar-macros` dependency, not here; benchmark *programs* exist but no harness, and none is required, since ADR-010's hasher was measured ad hoc under callgrind. The rule the survey lands on: **a staging decision enters the compiler only with a measured crossover.**
