...
*   [ ] **Tier-1 staging (compiler-side)**: independent of compile-time I/O and blocked by none of its questions ([ADR-022](specification/adr/adr-022.md) §3). Survey of real candidates, closed ones, and how to measure one: [`docs/staging-candidates.md`](staging-candidates.md). Headline findings: route hashing has no target (no HTTP server); the biggest win - dispatch over literal alternations - lives in the `winnow-grammar-macros` dependency, not here; benchmark *programs* exist but no harness, and none is required, since ADR-010's hasher was measured ad hoc under callgrind. The rule the survey lands on: **a staging decision enters the compiler only with a measured crossover.**
*   [ ] **Compile-Time I/O**: the producer the cache's asset dimension is waiting for. Not a straightforward feature - a grammar's `action` blocks are arbitrary Nikaia, so evaluating one at build time means running user code at build time. The design space is staked out in [ADR-022](specification/adr/adr-022.md) (**Open**): two things decided (I/O belongs to the compiler, not the sandbox; paths stay in the project root and `..` is refused rather than resolved), six questions listed, and the one that blocks the others named - what a program is allowed to do in `const`.
