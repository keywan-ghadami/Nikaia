...
| tuple, or-, range-, guarded, nested patterns in `match` | parse error on `(1, y) =>`. `calc.nika` matches `step.0` because it cannot match `step`. Six pattern shapes, none composable. *Decided since:* [ADR-137](specification/adr/adr-137.md) — the six, a guard is `if`, and `..` in a pattern is inclusive, so `..<` becomes the exclusive range everywhere and `..=` goes. **The range spelling is built**; the six shapes are not. |
