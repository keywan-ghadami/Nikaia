RFC 0.0.6 – Grammar Protocol, Scannerless Parsing & Typed Bindings
 ...
 * Delegation: The Core suspends execution and passes the current Byte Cursor to the specific Grammar implementation.
 * Smart Parsing: The Grammar parses the stream. It is responsible for detecting the logical end of its block and handling internal escaping (e.g., dsl text { ... } inside ... }) without Core intervention.
 * Resumption: The Grammar returns the cursor positioned immediately before the closing delimiter.
...
> let <Result> = dsl <Grammar> [ from <Source> | { <Body> } ]
> 
