# ADR-002: A Multi-Language Toolchain over the `rustc` Bridge

**Status:** **Partly superseded** by [ADR-003](adr-003.md) — the phase
architecture only. §1's Phase 0, `nikaia.lock` as a deterministic cache key, is
the starting point [ADR-021](adr-021.md) works out and is marked below.
**Date:** 2026-01-18
**Superseded in part by:** [ADR-003](adr-003.md) — the five-phase pipeline that
hijacks `rustc_driver` per language, replaced by Bridge-IR and one generic
orchestrator; and by [ADR-021](adr-021.md) D9 for Phase 4's Cranelift claim
**Still cited by:** [ADR-007](adr-007.md) (the multi-language objective),
[ADR-021](adr-021.md) (the cache key)

> **What survived.** ADR-003 replaced *how* a language reaches `rustc` — not the
> goal of hosting several, and not the observation that a build reading external
> files needs a deterministic key over what it read. The second became
> [ADR-021](adr-021.md). Everything else here is history.
...
* **Compile-Time Caching:** *(The germ of [ADR-021](adr-021.md), which settles what is
  actually hashed, what invalidates what, and where artifacts live. The sketch below is
  not normative; ADR-021 is.)* The `nikaia.lock` file acts as a deterministic cache key. If a macro performs Compile-Time I/O (e.g., reading an external file), the file's SHA256 hash is recorded. Subsequent builds bypass macro evaluation if the hash remains unchanged, ensuring instant builds.
...
* **Code Generation:**
  * `nikaia build` utilizes `-Z codegen-backend=cranelift` for sub-second development iterations.
    *Superseded by [ADR-021](adr-021.md) D9: this claim was never measured. Cranelift is compatible
    but buys ~1s on an incremental rebuild here, so it is a supported option rather than the default.*
