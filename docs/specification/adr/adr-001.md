...
The parser is implemented using the **`syn-grammar`** crate. This replaces the internal backend API with a robust, declarative parser generator.

**Goal:**
The strategic objective is to write the Nikaia Parser definition in a grammar file (e.g., `nikaia.g`) and fully compile a Nikaia Parser written in Nikaia (via this DSL).

**`syn-grammar` Features:**
We leverage `syn-grammar` to handle parsing boilerplate. Key features included in this architecture:
*   **EBNF Syntax:** Rules defined with sequences, alternatives (`|`), optionals (`?`), and repetitions (`*`, `+`).
*   **Type-Safe Actions:** Rust code blocks (`-> { ... }`) attached to rules to construct the AST directly.
*   **Syn Integration:** Native support for Rust identifiers (`ident`) and literals.
*   **Left Recursion:** Automatic handling of direct left recursion for expressions.
*   **External Files:** Support for keeping grammar definitions in `.g` files.

```rust
// parser.rs (using syn-grammar)
use syn_grammar::grammar;

grammar! {
    grammar CompilerGrammar {
        rule compilation_unit -> Vec<Stmt> = stmt*

        rule stmt -> Stmt = 
            | "let" name:ident "=" val:expr -> { Stmt::Let { name, value: val } }
            // ... other statements

        // Parsing Closures (Arrow Syntax)
        rule closure -> Expr = 
            | "move"? "{" body:block "}" -> {
                Expr::Closure { args: vec![], body, is_move: true }
            }
            | arg:ident "=>" body:expr -> {
                Expr::Closure { args: vec![arg], body, is_move: false }
            }
    }
}
```
