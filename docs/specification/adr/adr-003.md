...
**Status:** Accepted
**Date:** 2026-02-18
**Supersedes, in part:** [ADR-001](adr-001.md) (§1.1, §2.1's `driver` module,
Chapter 3) and [ADR-002](adr-002.md) (the five-phase pipeline). **Only the
coupling to `rustc` internals.** What those records say about the pinned
toolchain, the parser backend, the bootstrap stages and the cache key is
untouched and still in force — see the
[supersession section](README.md#what-supersedes-what).
**Related:** [ADR-004](adr-004.md) (how Bridge-IR reaches machine code),
[ADR-021](adr-021.md) (the orchestrator's build cache)
