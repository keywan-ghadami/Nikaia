...
| [078](adr-078.md) | **A trait is declared, and a bound is answered from the declaration.** Part I 4.7 writes `trait Summarize { … }` as its own example and it was a parse error at the keyword - a trait could be implemented and not declared, which [074](adr-074.md) §4 named as the prerequisite for `[T: Summarize]`. D3 is the reuse that made it small: a trait's methods are recorded as `Summarize::summary`, the same key shape an `impl`'s get, **because a bound and a receiver ask one question** - so the bound answers arity, argument types, result type, `sync` and `throws` from one entry rather than merely granting permission. D4 asserts `sync` for a declaration, because `async fn` in a trait is something the emitter cannot ask for and *absent* read as *pauses* | Accepted | **yes** - the word reserved, the item, `[T: A + B]`, and nine programs, one of which prints `User: Ada` |
...
| :--- | :--- | :--- |
| 079 | the bridge from build time to run time | `claude/mutex-boden-entscheidung-x291jd` |
