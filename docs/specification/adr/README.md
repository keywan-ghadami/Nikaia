...
| [040](adr-040.md) | A handle on a shared value is duplicated, never moved, and each handle dies at the end of its own block — so there is no operation to name and no second meaning for `.clone()` | Accepted | **partly** — built for a handle handed to a **function**, including that a borrow duplicates nothing, with `--sharing` naming each site. The **task** half is not: `spawn` lowers now, so a handle a task uses is *moved* into the body and `rustc` refuses the later use about the generated file. `open-work.md` carries it |
