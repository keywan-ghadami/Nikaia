...
> **Implementation status:** Partially implemented. `extern "C"` blocks, `unsafe { … }`, `NK1143` and the four view forms of [ADR-147](adr/adr-147.md) D1 are implemented, with `NK1158` for either form away from the boundary; the block above compiles and runs against libc. The length check of D1's neighbour ([ADR-147](adr/adr-147.md) D2), the opaque handle and its `cleanup` (D3) and the `CStr` copy (D4) are not implemented ([ADR-147](adr/adr-147.md) §5). `Pointer[u8]` is still refused with `NK1135`, which D5 makes permanent rather than pending.
