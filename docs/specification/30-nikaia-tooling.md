...
> **Implementation status:** Partially implemented. `extern "C"` blocks, `unsafe { … }`, `NK1143`, the four view forms of [ADR-147](adr/adr-147.md) D1 and the length check of D2 are implemented, with `NK1158` for either form away from the boundary and `NK1159` for a length that cannot be shown to fit; the block above compiles and runs against libc. The opaque handle and its `cleanup` (D3) and the `CStr` copy (D4) are not implemented ([ADR-147](adr/adr-147.md) §5). `Pointer[u8]` is still refused with `NK1135`, which D5 makes permanent rather than pending.
