...
> the compiler reports are `NK1101`–`NK1109` and `NK1111`–`NK1117`, `NK2202`,
> `NK2302`, `NK2501`,
> `NK2502`, `NK2605` and `NK2701`; `NK1110`, `NK2101`, `NK2102`, `NK2201`, `NK2301`,
...
> `NK1110` joined them without its check changing: the check is built and the
> **boundary** moved out from under it ([ADR-047](adr/adr-047.md) D1), so it is
> the one code here that has a reproduction test's worth of machinery and no
...
> **Status: built, and the program that reaches `NK2502` does not finish building.** The verdict is asked of the destination and the refusal fires on a program that annotates a `SharedMut[T]` or a `Locked[T]` and hands it to an undescribed call. Neither type is one the backend can build ([ADR-039](adr/adr-039.md) §4, Part II 12.2), so such a program is refused here and would fail to emit if it were not — the check is ahead of the construct, which is the right order. The `Held` in the two shapes below is still hypothetical for the same reason.
