...
extern "C" {
    ...
    fn fopen(path: ref [u8], mode: ref [u8]) -> FILE
...
At this boundary a `?` on a `ref mut T` or a `ref [T]` belongs to **what it points at** and not to the view: a view here lives for the call and is never absent, so `ref mut sqlite3?` is a slot that holds a handle or nothing. A plain `ref T?` is untouched and is 2.3's own nullable view.
...
extern "C" {
    fn getenv(name: ref [u8]) -> foreign::CStr
}
...
pub fn write(path: Path, root: Root, data: ref [u8]; append: bool = false, create: bool = true) throws
```
...
```nika
impl File {
    pub fn read(ref mut self, into: ref mut [u8]) -> i64 throws
    pub fn write(ref mut self, data: ref [u8]) -> i64 throws
