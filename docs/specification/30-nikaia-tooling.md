...
| `NK23xx` | Aliasing | `NK2301` cannot change a collection while looping over it (Part I, 6.8). `NK2302` a parameter written `&str` is kept past the call it was given in — stored in a field of the subject, in a struct the function hands back, or in a task (Part I, 6.6). A view inside a struct carries the buffer it points into ([ADR-008](adr/adr-008.md) D1) and a naked one does not, so the message names the struct form as the way out. It is **wider than the rule**: a call on the subject may or may not keep what it is given, nothing written down says which, and an analysis that fails open here emits Rust that does not compile — so it reports the call as keeping it ([ADR-010](adr/adr-010.md) D1's polarity). |
...
> them.
>
> `NK2302` is the one code here that is **not** covered by C.4's property: it
> refuses a read-only call on a field that holds views, because nothing written
> down tells that call from one that keeps what it is given. Every other check
> on this page says nothing where it knows nothing.
>
