...
# `artifact = "c-library"`, which exports every `pub extern "C" fn` with a body
# and generates the header (ADR-125 D1). With `target`, this is what ADR-062
# means by a target that lets foreign code call in.
# artifact = "program"
# The prefix of every exported symbol and constant: the package name with
# `-` written `_`, unless this says otherwise (ADR-128 D1).
# symbol-prefix = "hc"
...
| `crosses` | type | a value of this type **may cross a thread** (`true`), **may not** (`false`, which is what the crossing refusals fire on — [ADR-123](adr/adr-123.md)), or nothing was recorded (absent) ([ADR-005](adr/adr-005.md) §1 Group B, `NK25xx`). Written by hand — or by `nikaia describe`, which reads a foreign type's fields — and never inferred, because it only ever answers for a type whose parts this compiler cannot walk: a Nikaia `struct` records its `fields`, and the check walks those. Its absence is "nobody said" and not "it may not" — and "nobody said" is not permission, so the compiler will not put such a value on a thread of its own choosing |
...
> **Status:** built, except the example's own return type. `extern` and `unsafe`
> are reserved words **with constructs** ([ADR-124](adr/adr-124.md) D1), the
> block lowers to Rust's own and the call to Rust's own `unsafe`, and a call to
> an `extern` name **outside** one is `NK1143` — the boundary is written where it
> is crossed rather than once beside the declaration. A declaration is `sync` and
> carries no `throws` without writing either (D2): C has no suspension point, and
> a C function that sleeps *blocks a thread*, which is Part II 12.1's question
> and not this one. `touches` and `locks` are fail-closed, as a described crate's
> are (D4).
>
> **What the example above still needs is `Pointer[u8]`**, which is a type
> nothing declares and which that record leaves open (§4): a pointer that
> outlives what it points at is the one thing this language is built not to
> allow, and a type for one wants a lifetime story rather than a name. So
> `extern "C" { fn getpid() -> i32 }` is a program that compiles and runs, and
> `malloc` is not one yet — **the block above is refused by this compiler**, with
> `NK1135` naming the type, rather than reaching the backend as it used to.
> `tests/specification/EXPECTED.txt` records it that way.
...
The other direction is the same word with a body: `pub extern "C" fn greet(name: &str) -> String { … }`
is an **entry point**, and a build with `artifact = "c-library"` in `[build]` makes a shared and a
static library of the package and generates `<package>.h` from the ledger. What a C programmer sees
is decided by two rules — the caller owns the memory, and a call never surprises:
...
> **Status:** not built — `extern "C"` with a body is a parse error, there is no library artifact
> and no header generator ([ADR-125](adr/adr-125.md) §5); the five records on it are unbuilt with
> it. A native Node add-on, a fixed-size array field and the component model are what they leave open.
...
> **Status:** the **refusal** is built and the command is not. A call into a
> crate the manifest declares with `type = "rust"` and no
> `contracts/<crate>.contracts` describes is `NK2504`, once per crate and with
> the command in the message; a name the build did **not** declare is left alone,
> because refusing on one nobody declared would be C.4's correct program refused.
> A written **type** from such a crate counts as much as a call does. What is not
> built is `nikaia describe` itself (D2, D4) and the hash rule (D5) — so a
> description is written by hand today, which D5 expects anyway, and
> `examples/foreign-runtime/` ships one. [ADR-104](adr/adr-104.md) §5 is the rest
...
*   **`unsafe`:** this was the keyword's only specified use, and it was held
    open for the FFI work of 15.1 rather than being dropped. **That day has
    come**: `unsafe { … }` is a construct and the word is on Part I 2.1's list,
    which joined it the way that list says one does — with the construct, in the
    same change ([ADR-124](adr/adr-124.md) D1). What 15.1 writes it for is a
    call across the C boundary; what *this* section would write it for is an
    `asm` block, and that is still unbuilt.
...
> the compiler reports are `NK1101`–`NK1113`, `NK1115`–`NK1139` and `NK1141`–`NK1154` — every one
> of them except the **retired** `NK1114` and the unused `NK1120` — and
> `NK2101`, `NK2103`, `NK2104`, `NK2202`, `NK2203`–`NK2209`,
> `NK2302`, `NK2501`, `NK2502`, `NK2504`, `NK2605`, `NK2606` and
> `NK2701`; `NK2102`, `NK2201`, `NK2301`, `NK2401`, `NK2601`–`NK2604`
