...
> the compiler reports are `NK1101`–`NK1113` and `NK1115`–`NK1127` — every one
> of them except the **retired** `NK1114` and the unused `NK1120` — and
> `NK2101`, `NK2103`, `NK2104`, `NK2202`, `NK2302`, `NK2501`, `NK2502`, `NK2605` and
> `NK2701`; `NK2102`, `NK2201`, `NK2301`, `NK2401`, `NK2601`–`NK2604`,
> `NK2203`–`NK2205` and `NK2503` are specified ahead of the check that would
...
> **Status: none of the four is emitted, and the types they are about now
> exist.** `SharedMut[T]`, `Locked[T]` and the four doors are built
> ([ADR-064](adr/adr-064.md)); what is not is any of these checks — nothing
> refuses the nesting `NK2203` is for, nothing refuses I/O inside a door, and
> nothing refuses a direct assignment ([ADR-039](adr/adr-039.md) D2, D10, Part II
> 12.2 and 12.3). So the four are specified ahead of the checks rather than ahead
> of the construct, which is a shorter way to go than it was. The shapes below are
> what the codes print (C.3).
