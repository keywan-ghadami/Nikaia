...
pub macro Describe(def: StructDef) -> AstExpr {
    ...
    let print_statements = def.fields.map(field => {
        let fname = field.name
        
        // 'quote' creates a piece of code. 
        // We inject 'fname' and 'self.fname' into this code.
        quote {
            println("Value of " + fname + " is: " + self.fname)
        }
    })
...
fn main() {
    ...
    let handle1 = spawn({ process_image("a.jpg") })
    let handle2 = spawn({ process_image("b.jpg") })

    // 3. Join (Await)
    // We wait for the background threads to finish and retrieve the values.
    // While waiting, the current thread can steal work from others.
...
particles.par_iter().for_each(p => calculate_physics(p))
```
...
access_all(account_a, account_b, (a, b) => {
