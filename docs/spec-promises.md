...
| `a?.b` | I 3.5 | parse error: *expected `}`; found `?`* — **re-run, unchanged.** The `?` position is free (the parser never builds `Expr::Try`); what is missing is the `map`-or-`and_then` decision, which is a question about the field's type (`open-work.md` §2.4) |
