...
//! feature that needs a nightly with its `rustc-dev` component - the one named
//! in `bridge-toolchain/rust-toolchain.toml` and installed by
//! `scripts/bridge-toolchain.sh` (ADR-001 D5, which is why the rest of the
//! workspace is on stable). Turning the feature off is the default for a clone,
//! and since ADR-004 D4 such a build is a whole installation rather than a
//! reduced one: the default backend is `rust` here and everywhere.
//!
